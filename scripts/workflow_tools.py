...
def shift_nodes(workflow: dict, after_x: int, delta: int, after_y: Optional[int] = None) -> dict:
    """
    ...
    for node in workflow.get('nodes', []):
        ...
        if pos[0] > after_x:
            if after_y is None or pos[1] == after_y:
                ...
                nodes_shifted += 1
                print(f"  Shifted: {node['name']} -> [{node['position'][0]}, {node['position'][1]}]")

    print(f"Total shifted: {nodes_shifted} nodes")
    return workflow


def shift_nodes_by_name(workflow: dict, node_names: list[str], delta_x: int, delta_y: int = 0) -> dict:
    """
...
def update_connection(workflow: dict, from_node: str, to_node: str,
                      output_index: int = 0, input_index: int = 0) -> dict:
    """
...
def insert_node_between(workflow: dict, new_node: dict,
                        before_node: str, after_node: str) -> dict:
    """
...
def list_nodes(workflow: dict) -> None:
    ...
    print("\nNodes:")
    print("-" * 60)
    for node in sorted(workflow.get('nodes', []), key=lambda n: n.get('position', [0, 0])[0]):
        pos = node.get('position', [0, 0])
...
def main():
    ...
    # shift_nodes command
    shift_parser = subparsers.add_parser('shift_nodes', help='Shift nodes by X position')
    shift_parser.add_argument('workflow', help='Workflow JSON file')
    shift_parser.add_argument('--after-x', type=int, required=True, help='Shift nodes after this X')
    ...
    shift_parser.add_argument('--dry-run', action='store_true', help='Preview without saving')
    ...
    name_parser.add_argument('--nodes', nargs='+', required=True, help='Node names to shift')
    ...
    name_parser.add_argument('--dry-run', action='store_true', help='Preview without saving')
    ...
    conn_parser.add_argument('--from', dest='from_node', required=True, help='Source node')
    conn_parser.add_argument('--to', dest='to_node', required=True, help='Target node')
    conn_parser.add_argument('--dry-run', action='store_true', help='Preview without saving')
