...
5. **Сразу начать работу над задачей:**
   - Диагностика проблемы (для багов)
   - Исследование кодовой базы (для фич)
   - Реализация решения
   - Worktree = изоляция, не точка остановки

## Особый случай: существующий issue

Если `$ARGUMENTS` содержит `#<номер>` или `issue #<номер>`:
- НЕ создавать новый issue
- Использовать `./scripts/task-start.sh <номер>`
- Продолжить с шага 4
