...
### 3.2 Load Config (для basic plan)

**⚠️ ВАЖНО:** На basic plan n8n переменные окружения через Settings недоступны. Используйте Set node для хранения конфигурации.

- **Node:** Set
- **Name:** `Load Config`
- **Values to Set:**
  - `decision_engine_api_url`: `https://genomai.onrender.com`
  - `decision_engine_api_key`: `[ваш API_KEY из Render]` ⚠️ **СЕКРЕТНОЕ ЗНАЧЕНИЕ**

**Альтернатива (если не хотите хранить в workflow):**
- Можно хардкодить значения прямо в HTTP Request node (менее безопасно, но работает)
...
- **Node:** HTTP Request
- **Method:** POST
- **URL:** `={{ $('Load Config').first().json.decision_engine_api_url }}/api/decision`
- **Headers:**
  - `Content-Type: application/json`
  - `Authorization: Bearer {{ $('Load Config').first().json.decision_engine_api_key }}`

**⚠️ Альтернатива (хардкод для basic plan):**
Если не используете Set node, можно указать напрямую:
- **URL:** `https://genomai.onrender.com/api/decision`
- **Authorization:** `Bearer [ваш API_KEY]`
- **Body:**
```json
{
