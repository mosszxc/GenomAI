...
### 2.2.1 Idea Found Check

- **Node:** IF (typeVersion: 2.3)

**Проверка существования idea:**

**КРИТИЧЕСКИ ВАЖНО:** Supabase node может вернуть объект или массив, поэтому условие должно обрабатывать оба случая.
...
```json
{
  "conditions": {
    ...
    "conditions": [
      {
        "id": "found-check",
        "leftValue": "={{ $json.id || (Array.isArray($json) && $json.length > 0 && $json[0].id) || '' }}",
        "rightValue": "",
        "operator": {
          "type": "string",
...
**Логика:**
- Если `$json` — объект с `id` → возвращает `id` → TRUE ветка (idea найдена)
- Если `$json` — массив с элементами, у которых есть `id` → возвращает `id` → TRUE ветка
- Если `$json` — пустой массив или объект без `id` → возвращает пустую строку → FALSE ветка (idea не найдена)

**Выходы:**
- **TRUE (output[0]):** → Hard Constraints Check
- **FALSE (output[1]):** → Error Response + Emit DecisionAborted
