...
### 3.4 Load Config (для basic plan)
...
- **Table:** `={{ "config" }}`
- **Filters:**
  - `key` = `decision_engine_api_url` OR `key` = `decision_engine_api_key`

**Альтернатива (проще):**
- Загрузите все конфигурации без фильтров
- Используйте Function node для извлечения нужных значений

### 3.5 Extract Config Values (для basic plan)
...
return [{
  json: {
    ...
    // Сохраняем остальные данные из предыдущих узлов
    idea_id: $('Load Idea').first().json.id,
...
```json
{
  ...
  "fatigue_state": "={{ $json.fatigue_state }}",
  "death_memory": "={{ $json.death_memory }}"
}
```
