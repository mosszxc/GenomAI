...
- **Headers:**
  ...
  - `Authorization`: `Bearer [ваш API_KEY из Render]` ⚠️ **Замените на реальный ключ**

**⚠️ Для basic plan:**
Просто укажите URL и API key напрямую в HTTP Request node. Set node не обязателен, если значения не меняются часто.
- **Body:**
```json
{
