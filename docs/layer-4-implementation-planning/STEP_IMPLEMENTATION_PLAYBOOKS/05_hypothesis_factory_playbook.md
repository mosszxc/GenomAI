...
- После DecisionMade с `decision = approve` (внутри workflow)
...
**Workflow:** `CreativePipelineWorkflow` (Step 6)
**Файл:** `temporal/workflows/creative_pipeline.py`
**Task Queue:** `creative-pipeline`

### 2.1 Trigger

Автоматически после Step 5 (Decision Engine) если decision = approve

### 2.2 Load Idea Context

**Activity:** `load_idea_context`
...
**Activity:** `build_hypothesis_prompt`
...
**Параметры:**
- `temperature = 0.7` (фиксированное)
- `N = 1–3` (фиксировано)

### 2.5 Persist Hypotheses

**Activity:** `save_hypotheses`
**Таблица:** `hypotheses`
...
- не происходит: отбор "лучшей", фильтрация, скоринг
...
- Decision = reject
- workflow не генерирует гипотезы
