...
- Outcome не может существовать без Decision
...
- `system` — результат собственного решения системы (связан с Hypothesis)
- `user` — внешний креатив медиабайера (не связан с Hypothesis)

**Важно:**
- `origin_type` определяет, как Outcome используется в Learning Loop
- `system` outcome → causal updates (обновление confidence, fatigue, death memory)
...
- Outcome всегда ссылается на Decision
- Hypothesis всегда ссылается на Idea
- Decision всегда предшествует Outcome
