...
**❗ КРИТИЧЕСКОЕ ПРАВИЛО — ОБЯЗАТЕЛЬНО ДЛЯ РЕАЛИЗАЦИИ:**

**Outcome разделяется на два класса с жёсткими семантическими различиями:**

### 9.1.1 System Outcome (causal)

**Определение:**
Outcome с `origin_type = system`.

**Инварианты:**
- **может существовать только при наличии:**
  - Decision (обязательно, `decision_id IS NOT NULL`)
  - Hypothesis (обязательно, `hypothesis_id IS NOT NULL`)
- **используется для:**
  - learning (единственная сущность, из которой система обучается)
  - обновления confidence / fatigue
  - decision policy
- **единственная метрика успешности в MVP:** `CPA_window` (windowed CPA)
  - Формула: `CPA_window = total_spend(window) / total_conversions(window)`
  - Рассчитывается по агрегированному временному окну (D1–D3 или D1–D7)
  - Применяется только к System Outcome
  - Метрика считается только после закрытия окна

**Жёсткое правило:**
System Outcome не может существовать без Decision.

**Запрещено:**
- ❌ Learning на System Outcome без Decision
- ❌ Использование CTR, CVR, ROAS, Early CPA, Engagement metrics для learning/decision
- ❌ Real-time показатели как метрика успешности

### 9.1.2 User Outcome (observational)

**Определение:**
Outcome с `origin_type = user`.

**Инварианты:**
- **может существовать без Decision и Hypothesis:**
  - `decision_id` nullable (может быть NULL)
  - `hypothesis_id` nullable (может быть NULL)
- **не используется для:**
  - learning
  - decision
  - confidence / fatigue updates
- **используется исключительно для:**
  - observational memory
  - pattern discovery
  - расширения пространства идей
  - обновления similarity signals

**Жёсткое правило:**
User Outcome никогда не влияет на автоматические решения и обучение.

**Запрещено:**
- ❌ Learning на User Outcome
- ❌ Использование User Outcome в Decision Engine
- ❌ Обновление confidence/fatigue на основе User Outcome
...
- Требует Decision (не может существовать без Decision, `decision_id IS NOT NULL`)
- Требует Hypothesis (всегда связан с Hypothesis, `hypothesis_id IS NOT NULL`)
...
- **Единственная метрика успешности в MVP:** `CPA_window` (windowed CPA)
- **Запрещено:** использование CTR, CVR, ROAS, Early CPA, Engagement metrics для learning/decision

**User Outcome (`origin_type = user`):**
- Не требует Decision (может существовать без Decision, `decision_id` nullable)
- Не требует Hypothesis (не связан с Hypothesis, `hypothesis_id` nullable)
...
- **НЕ влияет** на causal learning loop
...
**Memory is not a primary stored entity.**

**Memory represents aggregated, derived state materialized from Outcome and Decision via Learning Loop.**

**Детали:**
- Memory не является первичной сущностью хранения (primary entity)
- Memory — это агрегированное производное состояние (derived state)
- Memory формируется Learning Loop на основе Outcome и Decision
- Memory не создаётся напрямую через INSERT в таблицу
- Memory не пишется напрямую из Decision Engine или других компонентов

**⚠️ Предупреждение для реализации (storage design):**

При реализации разработчик **НЕ должен**:
- ❌ создавать таблицу `memory` как primary entity
- ❌ пытаться "писать" в Memory напрямую из Decision Engine или других компонентов
- ❌ использовать Memory как источник истины для создания других сущностей

**Правильный подход:**
- ✅ Memory хранится как **материализованное представление** (materialized view) или **агрегированное состояние**
- ✅ Memory обновляется **только Learning Loop** на основе Outcome и Decision
- ✅ Memory читается Decision Engine, но не пишется им
- ✅ Memory является **derived state**, вычисляемым из первичных сущностей (Outcome, Decision)

**Почему это критично:**

- Без явного указания разработчик может создать таблицу `memory` и пытаться писать туда напрямую
- Это нарушит архитектуру: Memory должна быть производной от Outcome и Decision
- Это создаст риск несогласованности: Memory может не соответствовать реальным Outcome и Decision
- Правильный storage-дизайн: Memory = materialized view или computed state, а не primary table
