...
GenomAI использует 31 активный workflow для оркестрации пайплайна. Все workflow следуют event-driven архитектуре с emit событий в Supabase.

**Последнее обновление:** 2025-12-30
**Всего workflows:** 40
**Активных:** 31
**Неактивных (legacy/manual):** 9
...
| ID | Name | Trigger | Purpose | Tested |
|----|------|---------|---------|--------|
| `WMnFHqsFh8i7ddjV` | GenomAI - Creative Transcription | Webhook | Транскрипция видео через AssemblyAI | ✅ #93 |
| `pUjbKD9BUJktTdQ6` | GenomAI - Video Transcription | Webhook | Альтернативная транскрипция | ✅ |
...
### Telegram & Buyer System (Active)

| ID | Name | Trigger | Purpose | Tested |
|----|------|---------|---------|--------|
| `BuyQncnHNb7ulL6z` | Telegram Router | Telegram | Маршрутизация Telegram сообщений | ✅ #119 |
| `hgTozRQFwh4GLM0z` | Buyer Onboarding | Telegram | Онбординг новых байеров | ✅ #97 |
| `d5i9dB2GNqsbfmSD` | Buyer Creative Registration | Telegram | Регистрация креативов от байеров | ✅ #97 |
| `pL6C4j1uiJLfVRIi` | Spy Creative Registration | Webhook | Регистрация spy креативов (конкуренты) | ✅ #178 |
...
| `9nCezru94d0ABHh4` | Creative Reply Handler | Telegram | Обработка ответов на креативы | ✅ |
| `97cj3kRY6zzlAy0M` | Zaliv Session Handler | Telegram | Обработка /zaliv сессий | ✅ |
| `5tJA4rinnJgd1LFO` | Buyer State Cleanup | Schedule | ~~DEPRECATED~~ Merged into Pipeline Health Monitor | ❌ #185 |

### Historical Import (Active)
...
| `6tu8j4M4wvwi0pyB` | Buyer Historical URL Handler v2 | Telegram | Обработка video_url от buyer | ✅ |
| `UYgvqpsU3TMzb2Qd` | Historical Import Video Handler | Telegram | Обработка video_url из Telegram | ✅ #100 |

### Metrics & Outcomes (Active)
...
| `243QnGrUSDtXLjqU` | Outcome Aggregator (via API) | Webhook | Агрегация outcomes для Learning Loop | ✅ #98 |

### Recommendations & Delivery (Active)

| ID | Name | Trigger | Purpose | Tested |
|----|------|---------|---------|--------|
| `wgEdEqt2BA3P9JlA` | Daily Recommendation Generator | Schedule | Генерация ежедневных рекомендаций | ✅ |
| `QC8bmnAYdH5mkntG` | Recommendation Delivery | Webhook | Доставка рекомендаций в Telegram | ✅ #125 |
...
| `H1uuOanSy627H4kg` | Pipeline Health Monitor | Schedule | Мониторинг pipeline + buyer state reset (6h) | ✅ #185 |
| `dvZvUUmhtPzYOK7X` | creative_ingestion_webhook | Webhook | Legacy webhook ingestion | ✅ |

### Utilities (Active)

| ID | Name | Trigger | Purpose | Tested |
|----|------|---------|---------|--------|
| `fcnpPrj9sCFUqoWF` | TranslateText | Webhook | Перевод текста | ✅ |
| `zwtqav0d2R35zQot` | AudioTranscribe | Webhook | Транскрипция аудио | ✅ |

### Inactive / Legacy / Manual

| ID | Name | Status | Notes |
|----|------|--------|-------|
| `4fRlGSpAV2o9NUCo` | Performance Metrics Collector | ❌ | Legacy, replaced by Keitaro Poller |
...
| `x2Udd72hNe0ODduu` | premise_generator | ❌ | Development |
...
### Purpose
Доставляет рекомендации байерам в Telegram. Форматирует компоненты и confidence, обновляет статус.

### Flow
```
Webhook Trigger → Parse Webhook → Load Recommendation → Check Found
  ├─ [NOT FOUND] → (stop)
  └─ [FOUND] → Load Avatar → Format Message → Check Buyer
               ├─ [NO BUYER] → Emit DeliveryFailed
               └─ [HAS BUYER] → Load Buyer → Send Telegram → Update Status → Emit RecommendationDelivered
```
...
|----|------|------|
| `webhook-trigger` | Webhook Trigger | webhook |
| `parse-webhook` | Parse Webhook | function |
| `load-recommendation` | Load Recommendation | supabase |
| `check-found` | Check Found | if |
| `load-avatar` | Load Avatar | supabase |
| `format-message` | Format Message | function |
| `check-buyer` | Check Buyer | if |
| `load-buyer` | Load Buyer | supabase |
| `send-telegram` | Send Telegram Message | telegram |
| `update-status` | Update Status | supabase |
| `emit-event` | Emit RecommendationDelivered | supabase |
| `emit-no-buyer` | Emit DeliveryFailed | supabase |

### Events Emitted
- `RecommendationDelivered` - при успешной доставке
- `RecommendationDeliveryFailed` - при отсутствии buyer_id

### Message Format
```
📋 Рекомендация #R-XXXX

👤 Целевая аудитория: {avatar.name}
   {avatar.surface_desire}

🎯 Компоненты:
• Hook: confession (85%)
• Angle: pain (72%)
• Proof: testimonial (65%)
• Emotion: fear (78%)

✅ Тип: проверенная комбинация
📊 Средняя уверенность: 75%
...
### Trigger
```json
POST /webhook/recommendation-delivery-trigger
{
  "recommendation_id": "uuid"
}
```

**ВАЖНО:** Требует ручной активации webhook в n8n UI после создания.
...
### Purpose
Регистрация spy креативов (от конкурентов) через Telegram. Формат: `spy URL`. Проходит полный pipeline: Transcription → Decomposition → Idea → Decision Engine → Hypothesis.

### Key Differences from Buyer Creative
- `source_type: 'spy'` (не 'user')
- `tracker_id: null` (нет Keitaro трекинга)
- Передаёт `is_spy: true` в decomposition

### Flow
```
Webhook Trigger (router/spy-creative) → Parse Message → Check Buyer
  ├─ [NO] → Reply Not Registered
  └─ [YES] → Insert Creative (source_type='spy') → Emit SpyCreativeRegistered
             → Reply Spy Started → Call Transcription → Call Decomposition (is_spy: true)
             → Reply Complete
```
...
| `webhook-trigger` | Webhook Trigger | webhook |
| `parse-message` | Parse Message | code |
| `check-buyer` | Check Buyer | supabase |
| `buyer-exists` | Buyer Exists | if |
| `insert-creative` | Insert Creative | supabase |
| `emit-event` | Emit SpyCreativeRegistered | supabase |
| `reply-spy-started` | Reply Spy Started | telegram |
| `call-transcription` | Call Transcription | httpRequest |
| `call-decomposition` | Call Decomposition | httpRequest |
| `reply-complete` | Reply Complete | telegram |
| `reply-not-registered` | Reply Not Registered | telegram |
| `reply-error` | Reply Error | telegram |

### Events Emitted
- `SpyCreativeRegistered` - при регистрации spy креатива

### Input Format (Telegram)
```
spy https://drive.google.com/file/d/xxx/view
```
...
Флаг `is_spy` заставляет decomposition workflow установить `payload.source_type = 'spy'` независимо от LLM output.

### Cascade Effect
- Telegram Hypothesis Delivery показывает badge `🔍 [SPY-BASED]` для гипотез на основе spy креативов
- Learning Loop исключает spy креативы из Keitaro metrics (нет tracker_id)
